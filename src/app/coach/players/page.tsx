// app/coach/players/page.tsx - REMOVED AVAILABLE PLAYERS SECTION
'use client';
...
export default function CoachPlayersPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!coachProfile) {
        setError('Coach profile not found. Please contact administrator.');
        return;
      }

      if (coachProfile.role !== 'coach') {
        setError('You are not authorized as a coach.');
        return;
      }
      ...
      // 4. Find coach's team
      const currentTeam = teams?.find(t => t.id === coachProfile.team_id) || null;
      setCoachTeam(currentTeam);

      // 5. Get players on coach's team
      if (currentTeam) {
  ...
  const removePlayerFromTeam = async (playerId: string) => {
    ...
    if (!confirm(`Remove this player from ${coachTeam.name}?`)) return;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <p className="mt-4 text-gray-600 dark:text-gray-300">Loading player data...</p>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Manage Team Players</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            View and manage players on your team
          </p>
        </div>
        <div className="flex items-center gap-4">
          {coachTeam && (
            <div className="px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 rounded-lg">
              <span className="font-semibold">Team:</span> {coachTeam.name}
            </div>
          )}
          <button
            onClick={loadData}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {coachTeam ? `${coachTeam.name} Players` : 'My Team'} 
        ...
        {teamPlayers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-2">
              {coachTeam ? 'No players on your team yet' : 'You are not assigned to a team'}
            </p>
            <p className="text-sm text-gray-400 dark:text-gray-500">
              {coachTeam 
                ? 'Players will appear here once they are added to your team' 
          ...
          <div className="space-y-4">
            {teamPlayers.map((player) => (
              <div key={player.id} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {player.phone && (
                      <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">📞 {player.phone}</p>
                    )}
                  </div>
                  {coachTeam && (
                    <button
                      onClick={() => removePlayerFromTeam(player.id)}
                      className="px-3 py-1.5 text-sm bg-red-100 dark:bg-red-900 hover:bg-red-200 dark:hover:bg-red-800 text-red-700 dark:text-red-300 rounded-lg transition-colors"
